...
func (info *DbInfo) InitConnect() {
	...
	dsn := fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?charset=utf8mb4&parseTime=True", dbConf.User, dbConf.Password, dbConf.Host, dbConf.Port, dbConf.Database)

	if db, e := gorm.Open(mysql.Open(dsn), &gorm.Config{}); e != nil {
		log.Warn("load mysql fail", zap.String("host", dbConf.Host), zap.String("port", dbConf.Port), zap.String("db", dbConf.Database))
		info.Conn = nil
	} else {
		log.Warn("load mysql success", zap.String("host", dbConf.Host), zap.String("port", dbConf.Port), zap.String("db", dbConf.Database))
		if dbConf.Idle == 0 {
			dbConf.Idle = 5
		}
		if dbConf.Open == 0 {
